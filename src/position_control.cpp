...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber uwblocalPosition = nh.subscribe("uwb/uwb_position", 10, &uwb_position_callback);
  ros::Subscriber uwbVelocity = nh.subscribe("uwb/uwb_velocity", 10, &uwb_velocity_callback);//50Hz
  ...
  char name[6] = {'a','a','.','t','x','t'};
  char i = 0;
  std::ifstream lognamefile;
  lognamefile.open("/home/xuanlingmu/log/logname.txt");
  while (!lognamefile.eof())
  {
    lognamefile >> name[i];
    i++;if(i>2)i=0;
  }
  ...
  //std::cout<<logname;
  std::ofstream f;
  f.open("/home/xuanlingmu/log/logname.txt");
  f<<name[0]<<name[1];
  f.close();
...
void Mission::step()
{
  ...
  zCmd     = -target_offset.z;
  
  double attyaw = toEulerAngle(current_atti).z;
  log.open(logname,std::ios::app|std::ios::out);
  log <<gps_current_pos.x<<" "<<gps_current_pos.y<<" "<<gps_current_velocity.x<<" "<<gps_current_velocity.y<<" "
      <<uwb_current_pos.x<<" "<<uwb_current_pos.y<<" "<<uwb_current_velocity.x<<" "<<uwb_current_velocity.y<<" "<<current_local_pos.x<<" "<<current_local_pos.y<<" "
...
{
  rc = *msg;
  //gear button: chose uwb or GPS
  if(rc.axes[5] == -5000)
  {usegps = true; useuwb = false;}
  if(rc.axes[5] == -10000)
  {usegps = false; useuwb = true;}
...
void uwb_position_callback(const geometry_msgs::PointStamped::ConstPtr& msg)
{
  uwb_current_pos = msg->point;

  if(useuwb == true)
    {
    current_local_pos = uwb_current_pos;
    //ROS_INFO("CURRENT UWB %f, %f", current_local_pos.x,current_local_pos.y);
    }
}

void uwb_velocity_callback(const geometry_msgs::TwistStamped::ConstPtr& msg)
{
   uwb_current_velocity = msg->twist.linear;
   
   if(useuwb == true)
    current_velocity = uwb_current_velocity;
}
