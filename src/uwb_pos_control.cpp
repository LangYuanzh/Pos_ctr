...
{
  ros::init(argc, argv, "uwb_control_node");
  ros::NodeHandle nh;
  ...
  ros::Subscriber localPosition = nh.subscribe("uwb/uwb_position", 10, &local_position_callback);
  ros::Subscriber VelocityENUSub = nh.subscribe("uwb/uwb_velocity", 10, &velocity_callback);//50Hz
...
*/
  ...
  ROS_INFO("despos: %f ,%f, %f offset: %f, %f, %f desvel: %f, %f, %f ",target_position.x,target_position.y,target_position.z,target_offset.x,target_offset.y,target_offset.z,target_velocity.x,
  target_velocity.y,target_velocity.z);
  ...
  if(obtainControlPermission && (!obtain_control_result))
	  {
		  ...
		  if(takeoff_result)
		  {
			...
			square_mission.setTarget(target_X, target_Y, 20, 0);
...
{
    current_velocity = msg->twist.linear;
}
	 ...
	 {	switch(square_mission.state)
		{
		  ...
		  case 1:
		    ...
		    else
		    {
		      ...
		      square_mission.start_gps_location = current_gps;
		      square_mission.start_local_position = current_local_pos;
		      square_mission.setTarget(20, 20, 0, 0);
